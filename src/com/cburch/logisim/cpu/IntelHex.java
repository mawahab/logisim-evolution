...
public final class IntelHex {
	...
	public String readString(int address) {
		for (Memory mem : memorySegments) {
			if (address >= mem.address && address < mem.address + mem.data.length) {
				String retval = "";
				while (address < mem.address + mem.data.length
						&& Character.isLetterOrDigit((char) mem.data[address - mem.address])) {
	...
	public void processLine(String line) {
		...
		if ("00".equals(recordType)) {
			// Data Records
			//
			// The Intel HEX file is made up of any number of data records that
			// are terminated with a carriage return and a linefeed. Data
			...
			//
			// processing of this record is so easy, there is no need for a
			// separate function
			...
			// records and HEX386 records. These records contain the upper 16
			// bits (bits 16-31) of the data address. The extended linear
			...
			// linear address stored in the data field is saved and is applied
			// to subsequent records read from the Intel HEX file. The linear
			// address remains effective until changed by another extended
			// address record.
			//
			// The absolute-memory address of a data record is obtained by
			// adding the address field in the record to the shifted address
