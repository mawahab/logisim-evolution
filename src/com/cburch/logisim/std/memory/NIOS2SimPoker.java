...
public class NIOS2SimPoker extends InstancePoker {
...
//		initValue = data.value;
	...
	public void mousePressed(InstanceState state, MouseEvent e) {
		...
		Nios2Data data = (Nios2Data) state.getData();
		if (pos.x>Nios2Simulator.UP_X && pos.y>Nios2Simulator.UP_Y && pos.x<(Nios2Simulator.UP_X+15) && pos.y<(Nios2Simulator.UP_Y+15)) {
			System.out.println("UP Pressed !!!");
			data.setOffset(Math.max(data.getOffset()-1, 0));
		}
		if (pos.x>Nios2Simulator.DOWN_X && pos.y>Nios2Simulator.DOWN_Y && pos.x<(Nios2Simulator.DOWN_X+15) && pos.y<(Nios2Simulator.DOWN_Y+15)) {
			System.out.println("Down Pressed !!!");
			data.setOffset(Math.min(data.getOffset()+1, 31));
		}
	}
	...
	public void keyTyped(InstanceState state, KeyEvent e) {
		...
		int locX = (int) (pos.getX() - Nios2Simulator.OFFX_VAL-35) ;
		int locY = (int) (pos.getY() - Nios2Simulator.OFFY_VAL);

	
		Nios2Data data = (Nios2Data) state.getData();
		if(inZone) {
			int col = locX/Nios2Simulator.COL_WIDTH;
			int row = locY/Nios2Simulator.ROW_HEIGHT;
			if(row>0&&row<16&&col>=0&&col<=1) {
				...
				curValue = data.getReg(addr);
				curValue = (curValue * 16 + val) & dataWidth.getMask();

				data.setReg(addr, curValue);
			}
