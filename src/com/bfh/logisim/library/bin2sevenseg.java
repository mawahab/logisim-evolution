...
public class bin2sevenseg extends InstanceFactory{
	...
	public static final int Din = 7;

	public bin2sevenseg() {
		super("Bin_to_7_Segment_decoder", Strings.getter("Bin2SevenSegment"));
		...
		ps[Din] = new Port(10,80,Port.INPUT,4);
		...
		ps[Din].setToolTip(Strings.getter("Bin Value"));
		setPorts(ps);
	}
	...
	public void paintInstance(InstancePainter painter) {
		...
		painter.drawPort(Din, "Bin", Direction.SOUTH);
//		painter.drawBounds();
		for (int i = 0 ; i < 7 ; i++)
			painter.drawPort(i);
	...
	public void propagate(InstanceState state) {
		if (state.getPortValue(Din).isFullyDefined()&
			!state.getPortValue(Din).isErrorValue()&
			!state.getPortValue(Din).isUnknown()) {
	...
	private int bit(int value, int shmt) {
	...
	public boolean HDLSupportedComponent(String HDLIdentifier,
		...
		if (MyHDLGenerator == null)
			MyHDLGenerator = new bin2sevensegHDLGeneratorFactory();
