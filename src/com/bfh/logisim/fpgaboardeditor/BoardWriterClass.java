...
class BoardWriterClass {
	...
	public BoardWriterClass(BoardInformation BoardInfo, Image BoardImage) {
		try {
			...
			Element fpgainfo = boardInfo
					.createElement(BoardInformationSectionString);
			root.appendChild(fpgainfo);
			Comment comment = boardInfo
					.createComment("This section decribes the FPGA and its clock");
			fpgainfo.appendChild(comment);
			Element clkinfo = boardInfo
					.createElement(ClockInformationSectionString);
			clkinfo.setAttribute(ClockSectionStrings[0],
					Long.toString(BoardInfo.fpga.getClockFrequency()));
			...
			pull.setValue(PullBehaviors.Behavior_strings[BoardInfo.fpga
					.getClockPull()]);
			clkinfo.setAttributeNode(pull);
			Attr IOS = boardInfo.createAttribute(ClockSectionStrings[3]);
			IOS.setValue(IoStandards.Behavior_strings[BoardInfo.fpga
					.getClockStandard()]);
			...
			UnusedPins.setAttribute("PullBehavior",
					PullBehaviors.Behavior_strings[BoardInfo.fpga
							.getUnusedPinsBehavior()]);
			fpgainfo.appendChild(UnusedPins);
			Element Components = boardInfo
					.createElement(ComponentsSectionString);
			root.appendChild(Components);
			Comment Compcmd = boardInfo
					.createComment("This section describes all Components present on the boards");
			...
			Element BoardPicture = boardInfo
					.createElement(ImageInformationString);
			root.appendChild(BoardPicture);
			Comment Pictcmd = boardInfo
					.createComment("This section hold the board picture");
			BoardPicture.appendChild(Pictcmd);
			Element pictsize = boardInfo.createElement("PictureDimension");
			pictsize.setAttribute("Width",
					Integer.toString(BoardImage.getWidth(null)));
